%Ajuste de diferencias finitas hacia adelante 
%   dfd_adelante(f, x_i) 
...
function [f_prima, error_fp, f_2prima, error_f2p]=dfd_adelante(f, x_i, h, v_r1, v_r2)
  
  f_prima=(-f(x_i+2*h)+4*f(x_i+h)-3*f(x_i))/(2*h);
  f_2prima=(-f(x_i+3*h)+4*f(x_i+2*h)-5*f(x_i+h)+2*f(x_i))/(h^2);
